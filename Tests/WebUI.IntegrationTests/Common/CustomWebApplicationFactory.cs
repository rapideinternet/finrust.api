...
namespace FinRust.WebUI.IntegrationTests.Common
{
    public class CustomWebApplicationFactory<TStartup> : WebApplicationFactory<TStartup> where TStartup : class
    {
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            builder
                .ConfigureServices(services =>
                {
                    ...
                    var context = scopedServices.GetRequiredService<FinRustDbContext>();
        ...
        {
            return await GetAuthenticatedClientAsync("jason@FinRust", "FinRust1!");
        }
        ...
        private async Task<string> GetAccessTokenAsync(HttpClient client, string userName, string password)
        {
            ...
            var response = await client.RequestPasswordTokenAsync(new PasswordTokenRequest
            {
                ...
                ClientId = "FinRust.IntegrationTests",
                ClientSecret = "secret",

                Scope = "FinRust.WebUIAPI openid profile",
                UserName = userName,
                Password = password
            });
